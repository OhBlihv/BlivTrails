    ...
    if (process == 0) {
      try {
        ...
        if (particleData == null) {
          instance.getParticleStorage().create(new ParticleData(uuidBytes, pcfg.getParticle().toString(), pcfg
                  .getLength(), pcfg.getHeight(), pcfg.getColour()));
        } else {
      ...
      trailMap.put(uuid, new PlayerConfig(uuid, particleEff,
              particleData.getType(), particleData.getLength(), particleData.getHeight(), particleData.getColour()));
      if (vanishEnabled) {
            ...
            if (trailMap.containsKey(player.getUniqueId().toString())) {
              ...
              try {
                Bukkit.getServer().getScheduler()
                      .cancelTask(instance.getListener().getActiveTrails().get(player.getUniqueId().toString()));
