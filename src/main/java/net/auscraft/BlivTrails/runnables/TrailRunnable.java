...
public class TrailRunnable implements Runnable
{
	...
	/**
	 * 
	 * @param instance BlivTrails instance
	 * @param player Player object of the player receiving the trail
	 * @param pcfg PlayerConfig object of the player receiving the trail
	 * @param listener TrailListener instance
	 * @param rand A Random() Object that is outside this function in order to reduce/stop duplicate results
	 * @param option Global Option Array
	 */
		...
		{		
			if(listener.getTrailTimeLeft().get(uuid) > 0)
			{
				...
				else //Random
				{
					...
					//double xvar = variationCfg[0], yvar = variationCfg[1], zvar = variationCfg[2];
					...
					for(int i = 1;i < (length + 1);i++)
					{
						if(particle.hasProperty(ParticleProperty.COLORABLE))
						{
							Bukkit.getScheduler().runTaskLater(instance, 
								//public DisplayColourableRunnable(ParticleEffect particle, ParticleColor data, Location loc)
								new DisplayColourableRunnable(particle, data, player.getLocation().add(0.0D, height, 0.0D))
							, i * 5);
						}
						else
						{
							Bukkit.getScheduler().runTaskLater(instance, 
								//public DisplayRegularRunnable(ParticleEffect particle, float xOff, float yOff, float zOff, float speed, Location loc)
								new DisplayRegularRunnable(particle, xOff, yOff, zOff, speed, player.getLocation().add(0.0D, height, 0.0D))
							, i * 5);
						}
					}
				}
